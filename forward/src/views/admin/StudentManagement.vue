    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑学生' : '新增学生'" width="600px">
      <el-form :model="userForm" label-width="100px" :rules="userRules" ref="userFormRef">
        ...
        <el-form-item label="学号">
          <el-input v-model="userForm.student_no" placeholder="请输入学号" />
        </el-form-item>
...
const filterForm = reactive({ role: 'student', is_active: undefined, search: '' })
const pagination = reactive({ page: 1, pageSize: 10, total: 0 })
const users = ref([])

const userForm = reactive({
  username: '', password: '', first_name: '', last_name: '', student_no: '',
  role: 'student', email: '', phone: '', is_active: true
})
...
const openAddDialog = () => {
  isEdit.value = false
  Object.assign(userForm, { username: '', password: '', first_name: '', last_name: '', student_no: '', role: 'student', email: '', phone: '', is_active: true })
...
const editUser = (row) => {
  ...
  Object.assign(userForm, {
    ...
    student_no: row.student_no, role: 'student', email: row.email, phone: row.phone, is_active: row.is_active
  })
  dialogVisible.value = true
}
...
const toggleUserStatus = async (row, val) => {
  try {
    ...
    ElMessage.success(val ? '学生已启用' : '学生已禁用')
  } catch (error) {
...
const handleDeleteUser = async (row) => {
  try {
    await ElMessageBox.confirm(`确定要删除学生 "${row.username}" 吗？`, '确认删除', { type: 'warning' })
